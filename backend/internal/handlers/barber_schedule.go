...
func (h *BarberScheduleHandler) UpdateHour(c *gin.Context) {
	...
	if err != nil {
		BadRequest(c, "invalid_hour_id", "invalid hour_id")
		return
	}
	var body upsertHourBody
	if err := c.ShouldBindJSON(&body); err != nil {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			NotFound(c, "hour_not_found", "working hour not found")
			return
		}
...
func (h *BarberScheduleHandler) UpdateBreak(c *gin.Context) {
	...
	if err != nil {
		BadRequest(c, "invalid_break_id", "invalid break_id")
		return
	}
	var body upsertBreakBody
	if err := c.ShouldBindJSON(&body); err != nil {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			NotFound(c, "break_not_found", "break not found")
			return
		}
		BadRequest(c, "update_failed", err.Error())
		return
